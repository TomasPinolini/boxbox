...
- **Draft realtime UI (Slice 13b, tramo 1)**: `features/draft/` — `draft-socket.ts` (the one place that opens the `/draft` namespace socket, `connectDraftSocket(leagueId, token)`, mirroring `ApiClient` as the single door for HTTP), `useDraftState.ts` (hook: connects on mount, stores the initial `draft:state`, keeps merging `draft:update`/`draft:complete` while the screen stays open), `DraftPage.tsx` (route `/leagues/:id/draft` — status badge, round + whose turn it is, picks made so far with names resolved via `useMembers`/`useDrivers`/`useConstructors`). Read-only: no `draft:pick` emit yet, no timer countdown UI — that's tramo 2, not built.
...
- **Slice 13b tramo 2 — picks + timer from the draft UI**. The Regularidad epic; tramo 1 (connect + show live state) is done, the backend has been ready since Slice 6.
...
The `agent-browser` CLI is installed (see `~/.claude/skills/agent-browser/`). Useful now that **Slice 13a (Frontend bootstrap)** shipped a real UI to inspect — login, ligas, detalle de liga, pilotos, draft en vivo (solo lectura — Slice 13b tramo 1).
